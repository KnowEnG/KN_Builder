...
def get_sources(db, edges):
    return db.run("SELECT DISTINCT(file_id) FROM raw_line rl JOIN edge2line e2l ON rl.line_hash = e2l.line_hash WHERE e2l.edge_hash IN ('{}')".format("','".join(edge[0] for edge in edges)))
...
def main():
    ...
    bucket_dir = os.path.join(cls, args.edge_type, args.species)

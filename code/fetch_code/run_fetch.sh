...
CTR=1
CURL="curl -i -L -H 'Content-Type: application/json' -X POST -d@CODEDIR/chron_jobs/fetch_$SRC\_TMPNUM_TMPALIAS.json mmaster02.cse.illinois.edu:4400/scheduler/iso8601"
...
for ALIAS in `ls -d $SRC_DIR/* | sed "s#$SRC_DIR/##g" | sort`; do  
	...
	if [ "$RUN_TYPE" == "CONTAIN" ]; then
		...
		#CONTAIN_CMD=$(echo "docker rm check_TMPNUM_TMPSRC; docker run --name check_TMPNUM_TMPSRC -d -v $LOCAL_BASE/code/:/code/ -v $LOCAL_BASE/raw_downloads/:/raw_downloads/ $IMG $CMD" | sed -e "s/TMPNUM/$CTR/g" | sed "s#TMPBASE#/#g" | sed -e "s/TMPSRC/$ALIAS/g")
		#echo $CONTAIN_CMD
		#eval $CONTAIN_CMD
	fi
	...
	if [ "$RUN_TYPE" == "CLOUD" ]; then
		...
		cat $CODE_DIR/fetch_code/fetch_template.json | sed "s#IMG#$IMG#g" | sed "s#TMPCMD#$CMD#g" | sed "s#ALIASDIR#/data/#g" | sed "s#CODEDIR#/code/#g" | sed "s#ALIAS_DIR#$ALIAS_DIR/#g" | sed "s#CODE_DIR#$CODE_DIR/#g" | sed "s#CLOUDBASE#$CLOUD_BASE#g" | sed -e "s/TMPSRC/$SRC/g" | sed -e "s/TMPALIAS/$ALIAS/g" | sed -e "s/TMPNUM/$CTR/g" > $CODE_DIR/chron_jobs/fetch_$SRC\_$CTR\_$ALIAS.json;
		CLOUD_CMD=$(echo $CURL | sed "s#CODEDIR#$CODE_DIR/#g" | sed -e "s/TMPNUM/$CTR/g" | sed -e "s/TMPALIAS/$ALIAS/g")
		echo $CLOUD_CMD
		eval $CLOUD_CMD
	fi

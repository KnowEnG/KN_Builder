...
def run_step(step, wait=True):
    ...
    if step == 'MYSQL':
        args = ['python3', KNP_CODE_DIR + 'mysql_utilities.py',
                '-myh', KNP_MYSQL_HOST, '-myp', KNP_MYSQL_PORT,
                '-myps', KNP_MYSQL_PASS, '-myu', KNP_MYSQL_USER,
                '-wd', KNP_WORKING_DIR, '-dp', KNP_DATA_PATH,
                '-lp', KNP_LOGS_PATH, '-sd', KNP_STORAGE_DIR,
                "-mym", KNP_MYSQL_MEM, "-myc", KNP_MYSQL_CPU,
                "-myd", KNP_MYSQL_DIR, "-mycf", KNP_MYSQL_CONF,
                "-m", KNP_MARATHON_URL, "-mycu", KNP_MYSQL_HOST]
    elif step == 'REDIS':
        args = ["python3", KNP_CODE_DIR + 'redis_utilities.py',
                ...
                "-lp", KNP_LOGS_PATH, "-rcu", KNP_REDIS_HOST]
    elif step == 'SETUP':
        args = ['python3', KNP_CODE_DIR + 'workflow_utilities.py', 'CHECK', '-su',
                '-myh', KNP_MYSQL_HOST, '-myp', KNP_MYSQL_PORT,
                '-myps', KNP_MYSQL_PASS, '-myu', KNP_MYSQL_USER,
                '-rh', KNP_REDIS_HOST, '-rp', KNP_REDIS_PORT,
                '-wd', KNP_WORKING_DIR, '-dp', KNP_DATA_PATH,
                '-lp', KNP_LOGS_PATH, '-c', KNP_CHRONOS_URL,
                '-sd', KNP_STORAGE_DIR, '-es', KNP_ENS_SPECIES]
    elif step == 'CHECK':
        args = ['python3', KNP_CODE_DIR + 'workflow_utilities.py', 'CHECK',
                '-myh', KNP_MYSQL_HOST, '-myp', KNP_MYSQL_PORT,
                '-myps', KNP_MYSQL_PASS, '-myu', KNP_MYSQL_USER,
                '-rh', KNP_REDIS_HOST, '-rp', KNP_REDIS_PORT,
                '-wd', KNP_WORKING_DIR, '-dp', KNP_DATA_PATH,
        ...
        args = ['python3', KNP_CODE_DIR + 'workflow_utilities.py', 'IMPORT',
                '-myh', KNP_MYSQL_HOST, '-myp', KNP_MYSQL_PORT,
                '-myps', KNP_MYSQL_PASS, '-myu', KNP_MYSQL_USER,
                '-rh', KNP_REDIS_HOST, '-rp', KNP_REDIS_PORT,
                '-wd', KNP_WORKING_DIR, '-dp', KNP_DATA_PATH,
